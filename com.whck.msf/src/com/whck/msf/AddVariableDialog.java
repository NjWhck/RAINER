...
//		gridData.horizontalSpan = 1;
		...
		Button labelRadio=new Button(dbTypeGrp,SWT.RADIO);
		labelRadio.setText("文本标签");
		labelRadio.setAlignment(SWT.CENTER);
		labelRadio.setFont(font);
		labelRadio.setLayoutData(gridData);
		labelRadio.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				// TODO Auto-generated method stub
				if(labelRadio.getSelection()){
					dbType=4;
				}
			}
			public void widgetDefaultSelected(SelectionEvent e) {
	...
	protected void okPressed() {
		...
		if(Arrays.asList(Constants.DEFAULT_VALUE).contains(varName)){
			...
			xmlUtil.deleteVariable(zone.getName(),device.getName(), device.getId(), variable.getName());
		...
		xmlUtil.addVraiable(zone.getName(), device.getName(),device.getId(), variable);
		device.add(variable);
		showOK();
		super.okPressed();
	}
	...
	protected int checkForm(String varName, String unit,int maxVal, int minVal) {
		...
		if (xmlUtil.getVarNames(zone.getName(),device.getId(),device.getName()).contains(varName)) {
			return VALUE_EXIST;
		}
		if (unit.equals("")) {
			return UNIT_EMPTY;
